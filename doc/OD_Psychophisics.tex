...
% When we face a new research question, the problem of designing an experiment in order to address it arises. Typically, we start by making decisions about the number of participants, how many and what values of our independent variable should we test or how many times should we test each of those values. Most of the time, these choices are made on the basis of previous research on the field. However, it might be the case that there is not enough information on the literature to make these decisions with confidence, or that the values that are commonly used, do not allow for strong conclusions. Optimal Experimental Design (OED) offers an statistical alternative to solve this kind of problems.
